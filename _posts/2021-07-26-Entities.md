...
On Traction Rec we have written some code generation tooling to automatically generate these classes based on the metadata xml decribing objects, which we use throughout our products today. 
...
We can see that this inner Factory class provides a mechanism to construct a `StripeTransactionEntity` via the `wrap(Sobject record)` method.
...
A entity resolution strategy is just an interface that defines the following;
...
How often have you had to read a valuse from a picklist field and compare to a constant, and duplicate that same field comparison all over your code (or add a special utility method to define that comparison consistently across the code base)?
